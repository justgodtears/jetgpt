...
# Directiories
seq_file: Path = Path(__file__).parent.parent.parent.parent / "data" / "state" / "last_seq.txt"
raw_data_json: Path = Path(__file__).parent.parent.parent.parent / "data" / "raw" / f"data-{uuid.uuid4()}.json"
                ...
                async with websockets.connect(uri, subprotocols=['xrpc.v1.json']) as ws:
                    ...
                    async for frame in ws:
                        ...
                        if event["operation"] != "delete":
                            ...
                            count += 1
                            if count % 44000 == 0:
                                save_checkpoint(seq_file, last_seq)
...
async def main() -> None:
    ...
    with open(str(raw_data_json), "a") as opened_file:
        await listen(opened_file, seq_file)
